...
class Network( nn.Module ):
    ...
    def fit( self, x, y, epochs, batch_size, lr, loss_func ):
        self.train()    # What is train doing?
...
values = np.append( xx.ravel( ).reshape(x x.ravel( ).shape[ 0 ], 1),
                    yy.ravel( ).reshape(y y.ravel( ).shape[ 0 ], 1),
                   axis=1)
...
layer_widths  = [2, 1]       # Input is 2 dimensional, output is 1 dimensiona
layer_centres = [ 40 ]       # number of hidden layers


basis_func = rbf.gaussian   # Basis function is Gaussian
